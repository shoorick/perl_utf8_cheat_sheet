...
=pod
    ...
    Запись чисел в разных системах счисления
        0xFF     => 255   - шестнадцатиричная запись
        0377     => 255   - восьмеричная запись
...
# последовательность Unicode символов от 127 до 255, этот диапазон в Unicode занят символами Latin1.
...
# Бинарные данные с флагом utf8 занимают больше памяти
{
    ...
    is( bytes::length($bin), 6, "Установим для них флаг utf8, и объём данных вырастет до 6 байт" );
    ...
    is( bytes::length($bin), 36, "Возьмём бинарные данные размером 36 байт" );

    # Установим флаг
    utf8::upgrade($bin);

    is( bytes::length($bin), 72, "Установим для них флаг utf8, и объём данных вырастет до 72 байт" );

    # Снимем флаг
    utf8::downgrade($bin);

    is( bytes::length($bin), 36, "Снимем флаг, и объём станет снова равен 36 байт" );
}
